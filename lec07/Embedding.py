...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dim_embed, L):
        ...
        self.register_buffer(
            "pe", pe
        )  # 今回のPositionalEncodingは学習しない定数であることの指定(これ以降，クラス内からはself.peでアクセス可能(編集不要)

    def forward(self, x):  # x: batch_size x L x dim_embed

        return (
            x + self.pe
        )  # xとpeのサイズを考慮するとこの足し算がどうなるか考えてみること(参考: 課題1(d))
...
def plot_embedding(strs, pos, fname):
    ...
    for j in range(len(strs)):
        ...
        for i in range(len(strs[j])):
            plt.text(
                pos[j, i, 0], pos[j, i, 1], strs[j][i] + "(" + str(i) + ")", c=cols[j]
            )
    plt.legend()
    plt.savefig(fname)
    plt.close()
...
print(x)
plot_embedding(
    strs, x.detach().numpy(), "embed-1.pdf"
)  # x.detach().numpy()はxをnumpy配列に変換
