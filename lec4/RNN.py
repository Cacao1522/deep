...
d = 77  # 周波数チャネル数
m = 10  # クラス数(0-9の数字)
scaling_factor = 1e3  # スケーリングパラメータ
...
utterance_train = [
    ...
    6,
    7,
]  # 訓練用インデックス(作成時は少なく設定しておくとよい)
# utterance_test = list(range(8,11))  # テスト用インデックス
utterance_test = [8, 9, 0]  # テスト用インデックス
x_train, y_train, x_test, y_test = read_data(
    dir_name, utterance_train, utterance_test, scaling_factor
)
...
def adam(W, m, v, dEdW, t, alpha=0.001, beta1=0.9, beta2=0.999, tol=10 ** (-8)):
...
for epoch in range(0, num_epoch):
    ...
    for i in index:
        ...
        for t in range(T):
            ...
            Z_prime[:, t + 1], nabla_f[:, t] = forward(
                np.append(1, xi[t, :]), Z_prime[:, t], W_in, W, sigmoid
            )
        ...
        for t in reversed(range(T)):
            if t == T - 1:
                delta[:, t] = backward(
                    W, W_out[:, 1:], np.zeros(q), delta_out, nabla_f[:, t]
                )
            else:
                delta[:, t] = backward(
                    W, W_out[:, 1:], delta[:, t + 1], np.zeros(m), nabla_f[:, t]
                )
